...
def fetch_virustotal_info(ip_address, api_key):
    """Fetches information about an IP address from VirusTotal API."""
    headers = {
        "accept": "application/json",
        "x-apikey": api_key
    }
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(future_to_ip):
            ...
            if abuse_score > 0:  # Only proceed if reported
                result = fetch_virustotal_info(ip, virustotal_keys[0])  # Use the first key
                comment = get_comment(result)  # Get the comment based on the VirusTotal result
                results.append((ip, abuse_score, result, comment))
            else:
                results.append((ip, abuse_score, {}, "Clean"))  # No need to check VirusTotal if not reported

    # Write results to output CSV
    try:
        with open(output_path, 'w', newline='') as outfile:
            writer = csv.writer(outfile)
            # Write the original header plus new columns
            writer.writerow(header + ['AbuseIPDB Score', 'Malicious', 'Suspicious', 'Undetected', 'Harmless', 'Comment'])
            
            # Create a mapping of IP results for easy lookup
            ip_results = {ip: (abuse_score, result, comment) for ip, abuse_score, result, comment in results}
            
            for row in existing_data:
                ip = row[0]  # Assuming the IP is in the first column
                if ip in ip_results:
                    abuse_score, result, comment = ip_results[ip]
                    writer.writerow(row + [
                        abuse_score,
                    ...
                    writer.writerow(row + ['Not Reported', 'N/A', 'N/A', 'N/A', 'N/A', 'Clean'])
